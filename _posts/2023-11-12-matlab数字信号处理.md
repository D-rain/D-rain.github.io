...
`滤波器阶数N：`决定了滤波效果 ，理论上越大越好 但处理器的性能有限。
`系数设计方法：`
...
```
由于音频信号比较复杂，所以这里的滤波器系数较高。
但衰弱效果还是挺明显的,可以明显听出邓紫棋的高音也不高了 哈哈哈哈哈
Matlab仿真结果
...
# 记录
fir1()函数
`基本语法:` b = fir1(n,Wn) 输出为滤波器系数b,即前文提及的
&emsp;&emsp;&emsp;&emsp;&emsp;b = fir1(n,Wn,ftype)     
&emsp;&emsp;&emsp;&emsp;&emsp;b = fir1( ___,window)
&emsp;&emsp;&emsp;&emsp;&emsp;b = fir1( ___,scaleopt)

`fir1:` FIR滤波器 `fir2` IIR滤波器      
`n:` 滤波器阶数，指定为整数标量。     
`wn：`      
&emsp;• 如果 Wn 是标量，则设计的是截止频率为 Wn 的低通或高通滤波器。截止频率是滤波器的归一化增益为 -6 dB 处的频率。        
&emsp;• 如果 Wn 是二元素向量 [w1 w2]，其中 w1 < w2，则 fir1 设计的是截止频率下限为 w1 且截止频率上限为 w2 的带通或带阻滤波器。        
&emsp;• 如果 Wn 是多元素向量 [w1 w2 ... wn]，其中 w1 < w2 <…< wn，则 fir1 返回一个 n 阶多频带滤波器，频带为 0 < ω < w1、w1 < ω < w2、…、wn < ω < 1。             
`具体的` 
`归一化频率为滤波器的截止频率除以采样频率的一半。`
这样的好处是将频率与奈奎斯特频率频率归一化,这样滤波器就可以对`任意采样率`的信号都适用,
`ftype:`  滤波器类型,有以下参数可选,默认为'low'     
&emsp;&emsp;'low' | 'bandpass' | 'high' | 'stop' | 'DC-0' | 'DC-1'
`window:`  窗函数 默认为汉宁窗
![alt text](/img/blog/5-20-5.png)

# 具体应用
&emsp;通过仿真软件得到滤波器系数h[i] 然后与差分方程(信号转移函数)进行运算（在单片机或FPGA中完成），完成数字滤波器的设计。     
&emsp;其实这也是信号处理的普遍流程,即通过一些仿真软件,建立出相应的模型，通常是一个式子（这就是高大上的算法）,或者一组参数。然后利用一些嵌入式平台完成计算。
